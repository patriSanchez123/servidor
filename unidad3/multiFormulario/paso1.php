    ...
    if($error!=0){
       ...
       exit();
        ...
        if(!isset($_SESSION['productos']) && !isset($_SESSION['cesta'])){
            ...
            $consulta='SELECT * FROM producto LIMIT 4';
...
<body>
    ...
    <div id="paso1">
        ...
        $resultado=mostarObjetos($keys[0],$conexion);?>
...
<div id="paso2">
        <?php 
        $keys=array_keys($_SESSION['productos']);
        aniadirProductos($keys[0],$_POST['cantidad']);
        print_r($_SESSION['productos']);
        $resultado=mostarObjetos($keys[1],$conexion);?>
        <p>Inserte la cantidad que desea de este articulo</p>
        <form action="<?php $_SERVER['PHP_SELF'] ?>" method="post">
        <label for="producto2"><?php echo $resultado['nombre_corto']?>  <input type="number" id="producto2" name="cantidad"></label>
    ...
    //Insersión de datos paso3
        ...
        <?php 
        $keys=array_keys($_SESSION['productos']);
        aniadirProductos($keys[1],$_POST['cantidad']);
        print_r($_SESSION['productos']);
        $resultado=mostarObjetos($keys[2],$conexion);?>
        <p>Inserte la cantidad que desea de este articulo</p>
        <form action="<?php $_SERVER['PHP_SELF'] ?>" method="post">
        <label for="producto3"><?php echo $resultado['nombre_corto']?>  <input type="number" id="producto3" name="cantidad"></label>
