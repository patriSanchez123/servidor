...
function inicializarCesta($conexion){
...
function formularioProductos($conexion,$productosPorPágina,$paginaActual){
   ...
   $form.='<td><input type="number" name=cantidad['.$producto[0].'] value='.$_COOKIE[$producto[0]].'><input type="image" src="./fotos/shopping-cart-add-button_icon-icons.com_56132.png" name=agregarCesta['.$producto[0].']></input></td>';
   $form.='</tr>';
   }
   $form.='</table>';
   if($paginaActual!=1){
...
function aniadirACarrito($agregarCesta,$cantidad){
    $codProducto=array();
    $codProducto=array_keys($agregarCesta);
    setcookie($codProducto[0],$cantidad[$codProducto[0]]);
   
    

}

function insertarTodaPagina($cantidad){
...
function borraArticulo($eliminar,$cantidad,&$cesta){
    ...
    $eliminarProducto=$_COOKIE[$codProducto[0]]-$cantidad[$codProducto[0]];
    setcookie($codProducto[0],$eliminarProducto);
}

function imprimirCesta($conexion){
    $consultaClaves='SELECT cod FROM producto';
    $claves=$conexion->query($consultaClaves)->fetch_all();
    $clavesProductos=array();
    foreach($claves as $valor){
        array_push($clavesProductos,$valor[0]);
    }
    
    print_r($clavesProductos);
    ...
    foreach($_COOKIE as $clave=> $valor){
        if(in_array($clave,$clavesProductos)){
