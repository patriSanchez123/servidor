...
function calcularCuadrado($columna,$fila){
    ...
    /**
     * Funcion que muestra columna,fila,cuadrado,columna inicial,fila incial,columna final, fila final
     */
        function depuradorDeFunciones($sudoku){
            foreach($sudoku as $columna => $valorSudoku){
                foreach($valorSudoku as $fila =>$valor){
                       echo $fila . ',' . $columna . ' - '.CalcularCuadrado($columna,$fila) . 
                      ' ('.CalcularcolumnaInicialCuadrado($columna,$fila).','.CalcularFilaInicialCuadrado($columna,$fila).') - ('.
                          calcularColumnaFinalCuadrado($columna,$fila). ',' . CalcularFilaFinalCuadrado($columna,$fila) . ') | ';                            
                    }
                        echo '<br>'; 
                }
            }
