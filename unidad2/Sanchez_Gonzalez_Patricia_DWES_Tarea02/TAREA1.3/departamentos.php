...
error_reporting(E_ALL);
 ...
 $conexion = new mysqli($conexionBaseDatos['host'],$conexionBaseDatos['usuario'],$conexionBaseDatos['password'],$conexionBaseDatos['base de datos']);
...
<?php 
  ...
  echo '<pre>'.print_r($_POST).'</pre>';
    if(isset($_POST['delete'])){
      ...
      $clave = $clave[0];
      //Variable para comprobar que se haya relizado bien la sentencia
      $todoBien=true;
      //En este caso solo hace falta borrar departments ya que la base de datos tiene declarado borrado en cascada
      ...
      if($todoBien){
        $conexion->commit();
        echo '<p>Se ha Realizado el commit</p>';

      }else{
        $conexion->rollback();
        echo '<p>Se ha realizado un rollback</p>';
      }

      /**
       * Este código seria en el caso de que la base de datos no tuviera borrado en cascada
       */

      // $todoBien=true;
      // //Desactivamos el auto commit,para realizarlo cuando lo veamos oportuno
      // $conexion->begin_transaction();
      // if(!$conexion->query(('DELETE FROM dept_emp WHERE dept_no = \''.$clave.'\''))){
      //   echo '<p>No se ha podido borrar dept_emp</p>';
      //   $todoBien=false;}
      // if(!$conexion->query(('DELETE FROM dept_manager WHERE dept_no = \''.$clave.'\''))){
      //     echo '<p>No se ha podido borrar dept_manager</p>';
      //     $todoBien=false;}
      // if(!$conexion->query(('DELETE FROM departments WHERE dept_no = \''.$clave.'\''))||($conexion->affected_rows == 0) || !$todoBien){
      //   $mensaje_error_cliente = 'Se ha jodio el asunto colega ';
      //   $todoBien=false;
      // if($todoBien){
      //   $conexion->commit();
      //   echo 'Ha echo el commit';

      // }else{
      //   $conexion->rollback();
      //   echo 'Ha realizado un rollback';
      // }
